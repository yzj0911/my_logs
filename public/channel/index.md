...
2. 将数据直接发送给第一个等待的 goroutine， 唤醒接收的 goroutine
3. 如果 channel 的读等待队列不存在接收者goroutine
4. 如果循环数组buf未满，那么将会把数据发送到循环数组buf的队尾
5. 如果循环数组buf已满，这个时候就会走阻塞发送的流程，将当前 goroutine 加入写等待队列，并挂起等待唤醒
...
2. 如果是无缓冲 channel，直接从第一个发送者goroutine那里把数据拷贝给接收变量，唤醒发送的 goroutine
3. 如果是有缓冲 channel（已满），将循环数组buf的队首元素拷贝给接收变量，将第一个发送者goroutine的数据拷贝到 buf循环数组队尾，唤醒发送的 goroutine
4. 如果 channel 的写等待队列不存在发送者goroutine
5. 如果循环数组buf非空，将循环数组buf的队首元素拷贝给接收变量
6. 如果循环数组buf为空，这个时候就会走阻塞接收的流程，将当前 goroutine 加入读等待队列，并挂起等待唤醒
